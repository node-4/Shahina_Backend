const staticContent = require('../models/Static/staticContent');
const Faq = require("../models/Static/faqModel");
exports.createAboutUs = async (req, res) => {
        try {
                ...
                if (data) {
                        ...
                        const result = await staticContent.findByIdAndUpdate({ _id: data._id }, { $set: { image: image, link: link, designation: designation, title: title, description: description, type: data.type, } }, { new: true });
...
exports.createMembershipTerm = async (req, res) => {
        try {
                ...
                if (!data) {
                        ...
                        let terms = req.body.terms || data.terms;
                        const data1 = await staticContent.findByIdAndUpdate({ _id: data._id }, { terms: terms, type: data.type }, { new: true, });
                        return res.status(200).json({ status: 200, message: "update successfully.", data: data1 });
                }
        } catch (error) {
                console.log(error);
